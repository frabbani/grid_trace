...
static void test_reuse_create_destroy(void) {
  ...
  GridTr_destroy_reuse_array(&r);
  ASSERT_TRUE(r == NULL); // if your destroy sets NULL (recommended)
}

static void test_reuse_add_free_reuse_then_destroy(void) {
  struct GridTr_reuse_array_s *r =
      GridTr_create_reuse_array_(sizeof(int), 2, 2, __FILE__, __LINE__);
  ASSERT_TRUE(r);

  int a = 10, b = 11, c = 12;
  uint ia = GridTr_reuse_array_add(r, &a);
  uint ib = GridTr_reuse_array_add(r, &b);
  uint ic = GridTr_reuse_array_add(r, &c);

  ASSERT_EQ_U(ia, 0);
  ASSERT_EQ_U(ib, 1);
  ASSERT_EQ_U(ic, 2);

  GridTr_reuse_array_free(r, ib);
  ASSERT_TRUE(r->used[ib] == 0);
  ASSERT_EQ_U(r->avail_count, 1);

  int d = 99;
  uint id = GridTr_reuse_array_add(r, &d);
  ASSERT_EQ_U(id, ib);
  ASSERT_TRUE(r->used[id] == 1);

  int *pd = (int *)GridTr_array_get(r->array, id);
  ASSERT_TRUE(pd != NULL);
  ASSERT_EQ_I(*pd, 99);
...
static void test_reuse_double_free_ignored(void) {
  ...
  int v = 7;
  uint i0 = GridTr_reuse_array_add(r, &v);

  GridTr_reuse_array_free(r, i0);
  ASSERT_EQ_U(r->avail_count, 1);
...
static void test_reuse_free_out_of_range_ignored(void) {
  ...
  GridTr_reuse_array_free(r, 0);
  ASSERT_EQ_U(r->avail_count, 0);

  int v = 1;
  GridTr_reuse_array_add(r, &v);
  GridTr_reuse_array_free(r, 12345);
  ASSERT_EQ_U(r->avail_count, 0);
...
static void test_reuse_freelist_resizes(void) {
  struct GridTr_reuse_array_s *r =
      GridTr_create_reuse_array_(sizeof(int), 64, 64, __FILE__, __LINE__);
  ASSERT_TRUE(r);
...
static void test_reuse_add_returns_sequential_indices(void) {
  ...
  for (int i = 0; i < 10; i++) {
    uint idx = GridTr_reuse_array_add(r, &i);
    ...
    int *p = (int *)GridTr_array_get(r->array, idx);
    ASSERT_TRUE(p != NULL);
    ASSERT_EQ_I(*p, i);
...
static void test_reuse_free_then_reuse_lifo(void) {
  ...
  GridTr_reuse_array_free(r, ib); // free middle
  ASSERT_TRUE(r->used[ib] == 0);
  ASSERT_EQ_U(r->avail_count, 1);
...
static void test_reuse_free_out_of_range_is_ignored(void) {
  ...
  int v = 1;
  GridTr_reuse_array_add(r, &v); // num_elems = 1
  GridTr_reuse_array_free(r, 12345);
  ASSERT_EQ_U(r->avail_count, 0);
...
static void test_reuse_get_basic_live_and_freed(void) {
  ...
  int *pa = (int *)GridTr_reuse_array_get(r, ia);
  int *pb = (int *)GridTr_reuse_array_get(r, ib);
  int *pc = (int *)GridTr_reuse_array_get(r, ic);
  ...
  GridTr_reuse_array_free(r, ib);
  ASSERT_TRUE(GridTr_reuse_array_get(r, ib) == NULL);

  // other indices still live
  ASSERT_TRUE(GridTr_reuse_array_get(r, ia) != NULL);
  ASSERT_TRUE(GridTr_reuse_array_get(r, ic) != NULL);
...
static void test_reuse_get_out_of_range_and_null_array(void) {
  ...
  ASSERT_TRUE(GridTr_reuse_array_get(r, 0) == NULL);
  ASSERT_TRUE(GridTr_reuse_array_get(r, 1234) == NULL);

  int v = 7;
  uint i0 = GridTr_reuse_array_add(r, &v);

  // valid index returns non-null
  ASSERT_TRUE(GridTr_reuse_array_get(r, i0) != NULL);

  // out of range returns null
  ASSERT_TRUE(GridTr_reuse_array_get(r, i0 + 1) == NULL);

  // null array pointer returns null
  ASSERT_TRUE(GridTr_reuse_array_get(NULL, 0) == NULL);
